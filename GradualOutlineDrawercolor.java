...
public class GradualOutlineDrawercolor extends JPanel {
    ...
    public GradualOutlineDrawercolor(String imagePath) {
        try {
            ...
            for (int y = 1; y < height - 1; y++) {
                for (int x = 1; x < width - 1; x++) {
                    ...
                    edgeMatrix[x][y] = (edgeValue > 80); // Adjust the threshold as needed
                }
            }
        } catch (IOException e) {
...
public void drawOutline() {
    ...
    // Gradually draw the colored outline
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            if (edgeMatrix[x][y]) {
                // Get the color from the original image
                int originalColor = originalImage.getRGB(x, y);
                outlineImage.setRGB(x, y, originalColor); // Set the pixel color on the outline
                repaint();
                try {
                if(x%5==0)
    ...
    SwingUtilities.invokeLater(this::showCompletionOptions);
}
   ...
   private void showCompletionOptions() {
    ...
    switch (option) {
        case 0: // Save Outline
            ...
            if (result == JFileChooser.APPROVE_OPTION) {
                ...
                // Check for user-specified extension
                if (filePath.toLowerCase().endsWith(".jpg") || filePath.toLowerCase().endsWith(".jpeg")) {
                    format = "jpg";
                } else if (!filePath.toLowerCase().endsWith(".png")) {
                    filePath += ".png"; // Default to PNG if no valid extension provided
                }

                try {
                    ImageIO.write(outlineImage, format, new File(filePath));
                    JOptionPane.showMessageDialog(this, "Outline saved successfully as " + format.toUpperCase() + "!", "Success", JOptionPane.INFORMATION_MESSAGE);
                } catch (IOException e) {
    ...
    public static void main(String[] args) {
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            GradualOutlineDrawercolor panel = new GradualOutlineDrawercolor(selectedFile.getAbsolutePath());
